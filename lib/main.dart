...
// stateful
//상태가 있는 위젯 - 갑ㄴ 특성을 가집 - UI의 상태에 따라 상황이 바뀌는 위젯
//즉 상태가 변경되면 UI를 다시 랜더링할 수 있음 - build 다시 호출함
...
//상태를 유지해야 할 필요가 있는 위젯은 final로 선언함
  final String title;
...
//플러터앱의 상태관리와 UI 구현에 대한 내용 정의
class _MyHomePageState extends State<MyHomePage> {
  int _counter = 0;  //상태변수 정의

  void _incrementCounter() { //상태를 변화시키는 메서드 정의
    setState(() {
      // setSstate를 이용해서 플러터에게 상태 변화가 생김을 알림
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      // 앱화면에 다양한 위젯을 배치하고 렌더링할 수 있도록 도와주는 틀
      appBar: AppBar(
